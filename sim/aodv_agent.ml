...
object(s)
  ...
  method private local_repair ~src ~dst = 
    let fwhops = o2v (Rtab.hopcount ~rt:rtab ~dst:dst)
  ...
  *)
    ...
    match fresh with 
	  ...
	  let answer_rreq = 
	    ...
	    begin match (Rtab.seqno ~rt:rtab ~dst:rreq.Packet.rreq_dst) with 
	      | None -> false
	      | Some s when (Rtab.invalid ~rt:rtab ~dst:rreq.Packet.rreq_dst)
		  -> false
  ...
  method private send_rerr ~dst ~obo = (
    ...
    in
    let l3pkt =
      Packet.make_grep_rerr_l3pkt 
	~rerr_payload:adv
	~l3hdr:(
	  Packet.make_grep_l3hdr
	  ~srcid:owner#id
	  ~dstid:dst
	  ~flags:Packet.GREP_RERR
	  ~sseqno:seqno
	  ~shopcount:0
	  ~ttl:0 (* will be set by send_out *)
	  ()
  ...
  method private process_data_pkt 
	...
	s#hand_upper_layer ~l3pkt:l3pkt;
      ...
      if (Rtab.invalid ~rt:rtab ~dst:(Packet.get_l3dst ~l3pkt:l3pkt)) then (
	Grep_hooks.drop_data_rerr()
      ) else (
	  ...
	  with 
		...
		begin
		  ...
		  if (s#local_repair ~dst:dst ~src:(Packet.get_l3dst ~l3pkt:l3pkt)
		    ...
		    s#invalidate_route ~dst:dst;
		    s#send_rerr
		    ~dst:(Packet.get_l3src ~l3pkt:l3pkt)
		    ~obo:(Packet.get_l3dst ~l3pkt:l3pkt)
		  )
		end
	  )
      )
  ...
  method private send_rreq ~ttl ~dst ~dseqno ~dhopcount ~rerr = (
    ...
    if (Rtab.invalid ~rt:rtab ~dst:dst || s#packets_waiting ~dst:dst) then (
...
(*	      s#log_notice 
  ...
  method private invalidate_route ~dst = (
    Rtab.invalidate ~rt:rtab ~dst:dst;
    s#kill_buffered_packets ~dst:dst
  )
  ...
  method private process_rerr_pkt 
    ...
    ~(sender:Common.nodeid_t) = (
      ...
      let adv = (Packet.get_grep_rerr_pld ~l3pkt:l3pkt) in
      let invalid_dst = (adv.Packet.adv_dst) in
