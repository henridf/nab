...
let aodv_BLACKLIST_TIMEOUT =      (float aodv_RREQ_RETRIES) *. aodv_NET_TRAVERSAL_TIME
...
let aodv_RING_TRAVERSAL_TIME ttl_value =  
  2. *. aodv_NODE_TRAVERSAL_TIME *. (float (ttl_value + aodv_TIMEOUT_BUFFER))


(* Configuration parameters not defined in RFC *)
let aodv_PKTQUEUE_SIZE = 50 
