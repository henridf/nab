...
struct 
  ...
  let rel2abs__ cbuf rel = (
    let l = length_ cbuf in
      if rel >=  l then raise (Invalid_argument (Printf.sprintf "CircBuf.rel2abs__ : rel. offset (%d) greater than length (%d)" rel l));
      (cbuf.head + rel) mod l 
  )
  ...
  let get_ cbuf offset = try cbuf.buf.(rel2abs__ cbuf offset) with Invalid_argument "Array.get" -> raise (Invalid_argument "CircBuf.get_")
    
  (* Iterate, going from the head backward.
     Index i passed to f is the relative offset into array
  *)
  let iteri_ f cbuf = (
    let a2r = abs2rel__ cbuf in
